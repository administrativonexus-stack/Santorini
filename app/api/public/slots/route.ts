...
export async function GET(req: NextRequest) {
  ...
  const [{ data: wh }, { data: existing }, { data: blocks }] = await Promise.all([
    admin.from("working_hours").select("start_time, end_time")
      .eq("barber_id", barberId).eq("day", dayKey).eq("is_active", true).maybeSingle(),
    admin.from("appointments").select("scheduled_at, ends_at")
      .eq("barber_id", barberId).gte("scheduled_at", dayStart).lte("scheduled_at", dayEnd)
      .in("status", ["pending", "confirmed", "in_progress"]),
    admin.from("time_blocks").select("start_at, end_at")
      .eq("barber_id", barberId).gte("start_at", dayStart).lte("start_at", dayEnd),
  ]);
  ...
  (existing ?? []).forEach((apt) => {
    busy.push({ start: toLocalMinutes(new Date(apt.scheduled_at)), end: toLocalMinutes(new Date(apt.ends_at)) });
  });
  (blocks ?? []).forEach((blk) => {
    busy.push({ start: toLocalMinutes(new Date(blk.start_at)), end: toLocalMinutes(new Date(blk.end_at)) });
  });
